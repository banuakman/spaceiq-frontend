...
const fetchAvailableDeskByDate = async (selectedDate) => await api.getAvailableDeskByDate(selectedDate);

function SelectAnAvailableDesk({ selectedDate, handler }) {
  console.log(selectedDate);
  const { status, data, error } = useQuery(["bookingsByDate", selectedDate], () =>
  ...
  switch (status) {
    ...
    default:
      return (
        ...
        <form>
          <div className="mb-3">
              <select id="deskId" className="form-select" aria-label="available desks">
